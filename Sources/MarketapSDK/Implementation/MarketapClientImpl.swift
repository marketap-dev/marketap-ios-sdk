...
class MarketapClientImpl: NSObject, MarketapClient, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private func executeJSFunction(_ function: String, withArguments args: Any...) {
        ...
        let jsScript = """
        window.webkit.messageHandlers.print.postMessage('execute \(function)');
        await window._marketap_core.\(function)(\(formattedArgs));
        window.webkit.messageHandlers.print.postMessage('finish \(function)');
        """
                
        if webViewLoaded {
            print("### executeJSFunction", jsScript)
            webView.callAsyncJavaScript(jsScript, arguments: [:], in: nil, in: .page)
        } else {
            print("### pendingJSExecutions", jsScript)
