...
final class MarketapCache: MarketapCacheProtocol {
    ...
    func updateDevice(pushToken: String? = nil) {
        deviceQueue.async(flags: .barrier){
            ...
            Logger.verbose("[MarketapCache] updating device info: \(updatedDevice.toJSONString())")
    ...
    func saveCodableObject<T: Codable>(_ object: T, key: String) {
        do {
            ...
            Logger.error("[MarketapCache] Failed to save \(key): \(error.localizedDescription)")
        }
    }
    ...
    func loadCodableObject<T: Codable>(forKey key: String) -> T? {
        ...
        do {
            ...
            Logger.error("[MarketapCache] Failed to load \(key): \(error.localizedDescription)")
            return nil
        }
    }
